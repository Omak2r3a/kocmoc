...
information:CreateLabel("Place version: "..game.PlaceVersion)
information:CreateLabel("Updated by Omak2r3a")
...
information:CreateButton("Discord Invite", function() setclipboard("https://discord.gg/AFX7Yye8F8") end)
information:CreateButton("Donation", function() setclipboard("https://www.paypal.me/omak2r3a") end)
...
farmt:CreateToggle("Auto Meteor Shower", nil, function(State) kocmoc.toggles.meteorshower = State end)
farmt:CreateLabel(" ")
...
mobkill:CreateToggle("Auto Use Stinger", nil, function(State) kocmoc.toggles.autostinger = State end):AddToolTip("Uses 1 Stinger every 30 sec")

local serverhopkill = combtab:CreateSection("Serverhopping Combat")
serverhopkill:CreateButton("Vicious Bee Serverhopper [⚠️][📜]",function() loadstring(game:HttpGet("https://raw.githubusercontent.com/Omak2r3a/kocmoc/main/functions/viciousbeeserverhop.lua"))() end):AddToolTip("Serverhops for rouge vicious bees")
...
kocmocs:CreateButton("Overwrite Default Config", function() writefile("kocmoc/BSS_autoexec.json",game:service'HttpService':JSONEncode(kocmoc)) end):AddToolTip("Current config will be saved as BSS_autoexec.json and will be loaded automatically upon next execution")
...
aqs:CreateDropdown("Do NPC Quests", {'All Quests', 'Bucko Bee', 'Brown Bear', 'Riley Bee', 'Polar Bear', 'Black Bear', 'Spirit Bear', 'Science Bear'}, function(Option) kocmoc.vars.npcprefer = Option end)
...
task.spawn(function() while task.wait() do
        ...
        local outOfMicro = false
        if GetItemListWithValue()["Micro-Converter"] == 0 and kocmoc.toggles.automicro then outOfMicro = true end
        if GetItemListWithValue()["Micro-Converter"] > 0 and kocmoc.toggles.automicro then outOfMicro = false end
        if not kocmoc.toggles.automicro then outOfMicro = false end
        if kocmoc.toggles.automicro and not outOfMicro then --Checking if you have micros/enabled auto micro
...
task.spawn(function()
    while task.wait(1) do
        if GetItemListWithValue()["Stinger"] > 0 and kocmoc.toggles.autostinger then
        ...
        while task.wait(0.001) do
            if kocmoc.toggles.traincrab then
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
                    CFrame.new(-259, 111.8, 496.4) * CFrame.fromEulerAnglesXYZ(0, 110, 90)
                temptable.float = true
                temptable.float = false
            end
...
task.spawn(function() while task.wait(1) do
    ...
    if kocmoc.toggles.meteorshower then game.ReplicatedStorage.Events.ToyEvent:FireServer("Mythic Meteor Shower") end
